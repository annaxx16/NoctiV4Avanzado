...
function getConfiguredPrivateKey(): string | undefined {
  const privateKey = process.env.POLYMARKET_PRIVATE_KEY?.trim();
  if (!privateKey || PRIVATE_KEY_PLACEHOLDERS.has(privateKey)) return undefined;
  if (!/^0x[0-9a-fA-F]{64}$/.test(privateKey)) return undefined;
  return privateKey;
}

function getSdkPrivateKey(): string {
  const privateKey = getConfiguredPrivateKey();
  if (privateKey) return privateKey;
  if (CONFIG.dryRun) return DUMMY_PRIVATE_KEY;
  throw new Error('POLYMARKET_PRIVATE_KEY must be a valid 0x-prefixed 64-character hex key when DRY_RUN=false');
}
...
async function setupArbitrage(_sdk: PolymarketSDK) {
  ...
  arbService = new ArbitrageService({
    privateKey: CONFIG.dryRun ? undefined : getConfiguredPrivateKey(),
...
async function main() {
  ...
  if (!CONFIG.dryRun && !getConfiguredPrivateKey()) {
  ...
  dashboardEmitter.on('command', async ({ command, payload }: { command: string; payload: any }) => {
    ...
    if (command === 'redeemPosition') {
      ...
      try {
        const privateKey = getConfiguredPrivateKey();
        if (!privateKey) {
          log('WARN', 'Redeem requires a valid POLYMARKET_PRIVATE_KEY');
          return;
        }
